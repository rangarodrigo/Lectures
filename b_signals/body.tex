...
\begin{frame}[plain]{Periodicity of a Sinusoidal}
    ...
    A periodic continuous-time signal $x(t)$ has the property that  there is a positive values $T$ for which
    ...
    Fundamental period $T_0$ = smallest value of $T$ for which \ref{eq:period} holds.\\
...
\begin{frame}[plain]{Phase of a Sinusoidal}
    A time-shift in a CT sinusoidal is equivalent to a phase shift.\\
    E.g.: Show that a time-shift is a sinusoidal is equal to a phase shift.
    \mode<beamer>
    {
        \begin{equation*}
...
\begin{frame}[plain]{Phase of a Sinusoidal: $\phi = 0$}
    \mode<beamer>
    {
        \begin{columns}[t]
            ...
            \column{0.4\textwidth}
            {
                This signal is \alert{even}. If we reflect an even signal about the origin, it would look exactly the same.\\
                Periodic: $x(t) = x(t+ T)$.\\
                Even: $x(t) = x(-t)$.
            }
        \end{columns}
...
\begin{frame}[plain]{Phase of a Sinusoidal: $\phi = -\pi/2$}
    \mode<beamer>
    {
        \begin{columns}[t]
            ...
            \column{0.4\textwidth}
            {
                This signal is \alert{odd}. If we flip an odd signal about the time origin, we also multiply it by a ($-$) sign.\\
                Periodic: $x(t) = x(t+ T)$.\\
                Odd: $x(t) = -x(-t)$.
            }
        \end{columns}
    ...
    \mode<beamer>
    {
        \begin{columns}[t]
            ...
            \column{0.4\textwidth}
            {
                ...
                This signal is \alert{even}. \\
                Even: $x[n] = x[-n]$.\\
                Periodic: $x[n] = x[n+N]$, $N=16$\\
...
\begin{frame}{Phase Change and Time Shift in DT}
    ...
    \mode<beamer>
    {
        Answer: No.\\
        \begin{align*}
          A\cos[\omega_0(n+n_0)] &=  A\cos[\omega_0(n+n_0)]\\
        ...
        \begin{itemize}
          ...
          \item All continuous-time sinusoids are  periodic. However, discrete-time sinusoids are not necessarily so.
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Periodicity of a DT Signal}
    ...
    \mode<beamer>
    {
        ...
        $N$ may not be an integer. In this case, the sinal is not periodic.
    }
\end{frame}
