    ...
    \mode<beamer>
    {
        \begin{columns}
          ...
          \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}[remember picture,overlay]
              \node[draw=blue!50, fill=blue!20, inner sep=2pt,outer sep=2pt, rounded corners=0.1cm, anchor=south east, yshift=6cm, xshift=-1cm, text width=3cm, align=left]  at (current page.south east) {Euler's formula\\$e^{j\theta} = \cos\theta + j\sin\theta$\\$\cos \theta = \frac{e^{j\theta} + e^{-j\theta}}{2}$\\$\sin \theta = \frac{e^{j\theta} - e^{-j\theta}}{2j}$} ;
            \end{tikzpicture}
                \pause
